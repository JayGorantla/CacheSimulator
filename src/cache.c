...
Set *make_sets(int set_count, int line_count, int block_size) {
  ...
  for (int i = 0; i < set_count; i++) {
    ...
    set[i].lru_queue = (LRUNode *) malloc(sizeof(LRUNode));
    LRUNode *node = set[i].lru_queue;

    node->line = &set[i].lines[0];
    for (int i = 1; i < line_count; i++) {
      node->next = (LRUNode *) malloc(sizeof(LRUNode));
      node = node->next;
      node->line = &set[i].lines[i];
    }
